...
def main():
    ...
    eval_setup_1(file_path, target, center, size, angle, edge_width)


def eval_setup_1(file_path, target, center, size, angle, edge_width, show_mask=True):
    open_str = "Experiment configuration - Setup 1 (Bias, Precision)\n"
    ...
    for i in range(num_frames):
        ...
        nan_ratio[i] = (~valid_pixels).sum() / mask.sum()

        # Correct target to real size to get edges
        target.size = np.asarray(TARGET_SIZE)
        edge_precision[i, :] = get_edge_precision(target, depth_image, mean_depth,
                                                  extrinsic_params, intrinsic_params)
        target.size = size        
    ...
    print("Bias: {:0.5f}, Precision: {:0.5f}, at NaN-Ratio: {:0.5f}".format(
        total_bias, total_precision, total_nan_ratio))
    print("Edge Precision: {:0.5f} (left), {:0.5f} (down), {:0.5f} (right), {:0.5f} (up)".format(
        total_edge_precision[0], total_edge_precision[1],total_edge_precision[2],
        total_edge_precision[3]))

    cv2.destroyAllWindows()

    return total_bias, total_precision, total_nan_ratio, total_edge_precision, nan_edge_ratio, \
        first_image_with_target
...
def prepare_images(data, target, extrinsic_params, intrinsic_params):
    ...
    image_target     = target.show_target_in_image(disp, extrinsic_params, intrinsic_params)

    cv2.imshow("Image cropped with extended edges", target_cropped)
    cv2.imshow("Mask", image_mask)
    cv2.imshow("Image with target frame", image_target)
